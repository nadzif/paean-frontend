  ...
  contact.sections.map(data => {
    if (data.name === "contact") {
      data.properties.map(data => {
        if (data.key === "title") {
          title = lng === 'kr' ? data.value.kr : data.value.en;
        } else if (data.key === "address") {
          address = lng === 'kr' ? data.value.kr : data.value.en;
        } else if (data.key === "email") {
          email = lng === 'kr' ? data.value.kr : data.value.en;
        } else if (data.key === "phone") {
          phone = lng === 'kr' ? data.value.kr : data.value.en;
        }
      });
    }
  });
          ...
          <div className="container">
            <div className="row">
              <div className="col-lg-6 col-md-12 col-sm-12">
                <form data-request="{{ __SELF__ }}::onFormSubmit" action={`mailto:${email}`} method="post" enctype="text/plain">
                  <div className="row">
